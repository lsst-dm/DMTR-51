...
\begin{itemize}
  ...
  \texttt{ctrl{\_}pool} code to figure out these details.  
  ...
  to minimize the amount of integration effort.  In the area
  of the test running the exact same configuration as the 
  developer, the new Butler and SuperTask effort could 
  ...
  something different than the standard pipeline configuration.
...
hardware network problems.    The Network group replaced the
...
configuration test and actual full test).  To help production 
needs, special reservations were added to Slurm, and special 
...
filesystem, and prodution filesystems are being purchased
...
Strategy Document. 
  ...
  \item Butler issues \\
    ...
    \item The typical usage of Butler means all filenames are
      ...
      Operations.   This means that integration of the new Butler or
...
framework
  \begin{itemize}
    \item The step in DESDM which configures each step in each compute job does
...
software.
	\item The workflow definition (wcl) that describes a pipeline
      ...
      template.  For LDM-503-2 this is done manually
      ...
      template file from the pipeline definition would speed up the 
      integration process. 
  \end{itemize}
\end{itemize}
